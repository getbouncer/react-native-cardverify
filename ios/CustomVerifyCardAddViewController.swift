...
// MARK: Custom Verify
class CustomVerifyCardAddViewController: VerifyCardAddViewController {
    ...
    var manualCardEntryText = "Enter Card Manually"
    var manualCardEntryTextColor = UIColor.white
...
//        instructionUI()
    ...
    func handleVerifyStyleDictionary(viewStyle: NSDictionary?) {
        ...
        if let manualCardEntryText = dict["manualCardEntryText"] as? String {
            self.manualCardEntryText = manualCardEntryText
        }
        
        if let manualCardEntryTextColor = dict["manualCardEntryTextColor"] as? String {
            self.manualCardEntryTextColor = UIColor.init(hexString: manualCardEntryTextColor)
        }
    }
    ...
    override public func setupTorchButtonConstraints() {
        ...
        let bottomConstraints = [
            torchButton.topAnchor.constraint(equalTo: CustomVerifyCardAddViewController.manualCardEntryButton.bottomAnchor, constant: 15.0),
    ...
    override func setUpManualCardEntryButtonUI() {
        ...
        let text = self.manualCardEntryText
        let attributedString = NSMutableAttributedString(string: text)
        attributedString.addAttribute(NSAttributedString.Key.underlineColor, value: manualCardEntryTextColor, range: NSRange(location: 0, length: text.count))
        attributedString.addAttribute(NSAttributedString.Key.foregroundColor, value: manualCardEntryTextColor, range: NSRange(location: 0, length: text.count))
        attributedString.addAttribute(NSAttributedString.Key.underlineStyle, value: NSUnderlineStyle.single.rawValue, range: NSRange(location: 0, length: text.count))
        let font = CustomVerifyCardAddViewController.manualCardEntryButton.titleLabel?.font.withSize(20) ?? UIFont.systemFont(ofSize: 20.0)
        attributedString.addAttribute(NSAttributedString.Key.font, value: font, range: NSRange(location: 0, length: text.count))
        
        CustomVerifyCardAddViewController.manualCardEntryButton.setAttributedTitle(attributedString, for: .normal)
    }
